...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	bool vir_enough=1;
	if (pro->bp+numpages*PAGE_SIZE>(1<<ADDRESS_SIZE)) vir_enough=0;

	//-------------------------------------------------------
	if (phy_enough!=vir_enough) mem_avail=0;
	else {
		if (phy_enough==1 && vir_enough==1) mem_avail=1;
	}
	if (mem_avail==1) {
...
int free_mem(addr_t address, struct pcb_t * proc) {
	...
	if (translate(address,&addr_phy,proc)!=0){
		...
		while (page_phy!=-1){
			...
			for (i=0;i<proc->seg_table->table[seg_index].pages->size;i++){
				if (proc->seg_table->table[seg_index].pages->table[i].p_index==page_phy){
					proc->seg_table->table[seg_index].pages->table[i].v_index=0;
					proc->seg_table->table[seg_index].pages->table[i].p_index=0;
				}
			}
